...
export function EnrollmentModal({ userId, onComplete }: EnrollmentModalProps) {
    ...
    return (
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/40 backdrop-blur-sm">
            <div className="bg-white rounded-3xl shadow-2xl w-full max-w-lg mx-4 overflow-hidden animate-in fade-in zoom-in-95 duration-300">
                {/* Header */}
                <div className="bg-gradient-to-br from-indigo-600 to-blue-600 p-8 text-white">
                    <div className="flex items-center gap-3 mb-3">
                        ...
                        <span className="text-sm font-bold uppercase tracking-widest text-white/70">Welcome to CurricuLab</span>
                    </div>
                    <h2 className="text-2xl font-black mb-1">Select Your Class</h2>
                ...
                <div className="p-6">
                    {isLoading ? (
                        ...
                        <div className="space-y-3">
                            {programs.length === 0 ? (
                                <p className="text-center text-gray-400 py-8">No programs available. Contact admin.</p>
                            ) : programs.map(program => (
                                <button
                                    key={program.id}
                                    onClick={() => handleSelectProgram(program)}
                                    className="w-full flex items-center gap-4 p-4 rounded-2xl border-2 border-gray-100 hover:border-indigo-300 hover:bg-indigo-50 transition-all text-left group"
                                >
                                    <div className="w-12 h-12 bg-indigo-100 rounded-xl flex items-center justify-center group-hover:bg-indigo-200 transition-colors shrink-0">
                                        <Icons.GraduationCap size={22} className="text-indigo-600" />
                                    </div>
                                    <div className="flex-1 min-w-0">
                                        <p className="font-bold text-gray-900">{program.name}</p>
                                        <p className="text-sm text-gray-400 font-mono">{program.code}</p>
                                        {program.description && (
                                            <p className="text-xs text-gray-400 mt-0.5 truncate">{program.description}</p>
                                        )}
                                    </div>
                        ...
                        <div className="space-y-3">
                            <button
                                ...
                                className="flex items-center gap-1.5 text-sm text-gray-400 hover:text-gray-700 transition-colors mb-1"
                            >
                            ...
                            {semesters.length === 0 ? (
                                <p className="text-center text-gray-400 py-8">No active semesters in this program. Contact admin.</p>
                            ) : semesters.map(sem => (
                                <button
                                    key={sem.id}
                                ...
                                >
                                    <div className={cn(
                                        ...
                                        <span className={cn("font-black text-lg", selectedSemester?.id === sem.id ? "text-white" : "text-gray-500")}>
                                            {sem.number}
                                        </span>
                                    </div>
                                    <div className="flex-1 min-w-0">
                                        <p className="font-bold text-gray-900">{sem.shortName}</p>
                                        <p className="text-sm text-gray-400">{sem.name}</p>
                                        {sem.academicYear && (
                                            <p className="text-xs text-gray-400 mt-0.5">AY {sem.academicYear}</p>
                                        )}
                                    </div>
                                    {selectedSemester?.id === sem.id && (
                            ...
                            <button
                                ...
                                className="w-full mt-2 flex items-center justify-center gap-2 px-6 py-3.5 bg-indigo-600 hover:bg-indigo-700 disabled:bg-gray-200 disabled:cursor-not-allowed text-white font-black rounded-2xl transition-all shadow-lg shadow-indigo-200 active:scale-95"
                            >
                                {isSaving ? (
